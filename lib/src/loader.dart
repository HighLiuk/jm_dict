...
class JMDictLoader {
  bool _popAndInsert(
    ...
    final ids = store.box<JMDictEntryImpl>().putMany(
          entries,
        );
    final end = DateTime.now();
    final elapsedTime = (end
            .difference(
              start,
            )
            .inMilliseconds
  ...
  String _handleStartElementEvent(
    ...
    switch (name) {
      ...
      case "k_ele":
        ...
        (entry["k_ele"] as List).add(
          <String, dynamic>{},
        );
        break;
      case "r_ele":
        entry["r_ele"] ??= <Map<String, dynamic>>[];
        (entry["r_ele"] as List).add(
          <String, dynamic>{},
        );
        break;
      case "sense":
        entry["sense"] ??= <Map<String, dynamic>>[];
        (entry["sense"] as List).add(
          <String, dynamic>{},
        );
        break;
      case "re_nokanji":
  ...
  Future<void> load(
    ...
    await xmlFile
      ...
      onEndElement: (event) {
        final resultCode = _handleEndElementEvent(
          event,
          entry,
        );
        if (resultCode == 0) {
          final dictEntry = JMDictEntryImpl.map(
            entry,
          );
          if (dictEntry != null) {
            entries.add(
    ...
    final elapsedTime = (end
        ...
        1000;
    JMDictLogUtils.log(
      "JMdict loaded in $elapsedTime seconds",
    );
  }
  ...
  Future<void> loadFromAsset(
    ...
    final gz = await rootBundle.load(
      assetPath,
    );
...
class _JMDictDownloader {
  ...
  Future<Uint8List?> download() async {
    ...
    final gzPath =
        "${(await getTemporaryDirectory()).uri.toFilePath()}JMdict.gz";
    JMDictLogUtils.debugLog(
      "Downloading into $gzPath...",
    );
...
class JMDictUpdater {
  static Future<Uint8List?> _downloadAndProcessLatestFile(
    int? timeout, [
    String? downloadUrl,
  ]) async {
    return _JMDictDownloader(
      timeout,
      downloadUrl,
    ).download();
  }
  ...
  Future<bool> update({
    ...
    if (archivedBytes != null) {
      JMDictLogUtils.log(
        "Unarchiving JMdict.gz...",
      );
      final dictFile = await _unarchive(
        archivedBytes,
      );
      if (dictFile != null) {
        JMDictLogUtils.log(
          "Loading downloaded JMdict.gz...",
        );
        await loader.load(
          dictFile,
          store,
