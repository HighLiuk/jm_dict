...
extension _IterableChecker<T> on Iterable<T> {
  List<T>? get nullIfEmpty => this.isEmpty
      ? null
      : this.toList(
          growable: false,
        );
...
extension _StringListDelimiter on Iterable<String> {
  String? get obJoin => this.nullIfEmpty?.join(
        DELIMITER,
      );

  String get obJoinNonNull => this.join(
        DELIMITER,
      );
}

final _htmlTrimRegex = RegExp(
  r"[&;]|(::)|(=>)|(&&)",
);
final _delimiterTrimRegex = RegExp(
  r"(::)|(=>)|(&&)",
);

extension _StringCleaner on String {
  String get trimHtml => this.replaceAll(
        _htmlTrimRegex,
        "",
      );
  String get trimDelimiter => this.replaceAll(
        _delimiterTrimRegex,
        "",
      );
}
...
class JMDictEntryImpl implements JMDictEntry {
  ...
  @override
  @Transient()
  Set<KanjiElement>? get kanjiElements => _kanjiElements != null
      ? Set.unmodifiable(
          _kanjiElements!,
        )
      : null;

  static Map<int, Set<String>>? _parseMultiValue(
    String? data,
  ) {
    if (data != null) {
  ...
  void _prepareKanjiElements() {
    final keb = this.kanjiTexts?.split(
          DELIMITER,
        );
    if (keb != null) {
      final keInf = _parseMultiValue(
        this.kanjiInfo,
      );
      final kePri = _parseMultiValue(
  ...
  @override
  @Transient()
  Set<ReadingElement> get readingElements => Set.unmodifiable(
        _readingElements ?? {},
      );

  void _prepareReadingElements() {
    final reb = this.kanaTexts.split(
          DELIMITER,
        );
    final rEle = <ReadingElement>{};
    final readingRestrictions = _parseMultiValue(
      this.readingRestrictions,
    );
    final reInf = _parseMultiValue(
      readingInfo,
    );
    final rePri = _parseMultiValue(
      readingPriorities,
    );
    for (int i = 0; i < reb.length; i++) {
      ...
      rEle.add(
        ReadingElement(
          element,
          readingRestrictions?[i],
          reInf?[i],
          rePri?[i],
          noKanjiInfo?.contains(i) == true,
        ),
      );
    }
  ...
  @override
  @Transient()
  Set<SenseElement> get senseElements => Set.unmodifiable(
        _senseElements ?? {},
      );

  void _prepareSenseElements() {
    final senses = <SenseElement>{};
    final parsedGlossaries = _parseMultiValue(
      glossaries,
    );
    final stagK = _parseMultiValue(
      kanjiSenseRestrictions,
    );
    final stagR = _parseMultiValue(
      readingSenseRestrictions,
    );
    final xRef = _parseMultiValue(
      crossReferences,
    );
    final ant = _parseMultiValue(
      antonyms,
    );
    final pos = _parseMultiValue(
      partOfSpeeches,
    );
    final field = _parseMultiValue(
      fields,
    );
    final misc = _parseMultiValue(
      miscellaneous,
    );
    final sInf = _parseMultiValue(
      senseInfo,
    );
    final lSource = _parseMultiValue(
      sourceLanguages,
    );
    final dial = _parseMultiValue(
      dialects,
    );
      ...
      if (entrySequence != null && entrySequence is String) {
        ...
        if (entrySequence != null) {
          ...
          for (int i = 0; i < kanjiElements.length; i++) {
            ...
            kanjiInfo.addAll(
              _element["ke_inf"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimHtml}",
                  ) ??
                  [],
            );
            kanjiPriority.addAll(
              _element["ke_pri"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimDelimiter}",
                  ) ??
                  [],
            );
          }

          final List<Map<String, dynamic>> readingElements =
              map["r_ele"] as List<Map<String, dynamic>>? ??
                  <Map<String, dynamic>>[];
          ...
          for (int i = 0; i < readingElements.length; i++) {
            ...
            readingRestrictions.addAll(
              _element["re_restr"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimDelimiter}",
                  ) ??
                  [],
            );
            readingInfo.addAll(
              _element["re_inf"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimHtml}",
                  ) ??
                  [],
            );
            readingPriority.addAll(
              _element["re_pri"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimDelimiter}",
                  ) ??
                  [],
            );
          ...
          final List<Map<String, dynamic>> senses =
              map["sense"] as List<Map<String, dynamic>>? ??
                  <Map<String, dynamic>>[];
          ...
          for (int i = 0; i < senses.length; i++) {
            ...
            kanjiSenseRestrictions.addAll(
              _element["stagk"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimDelimiter}",
                  ) ??
                  [],
            );
            readingSenseRestrictions.addAll(
              _element["stagr"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimDelimiter}",
                  ) ??
                  [],
            );
            pos.addAll(
              _element["pos"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimHtml}",
                  ) ??
                  [],
            );
            xRef.addAll(
              _element["xref"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimDelimiter}",
                  ) ??
                  [],
            );
            ant.addAll(
              _element["ant"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimDelimiter}",
                  ) ??
                  [],
            );
            fields.addAll(
              _element["field"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimHtml}",
                  ) ??
                  [],
            );
            misc.addAll(
              _element["misc"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimHtml}",
                  ) ??
                  [],
            );
            sInf.addAll(
              _element["s_inf"]?.map<String>(
                    (String e) => "$i$IDX_DELIMITER${e.trimDelimiter}",
                  ) ??
                  [],
            );
